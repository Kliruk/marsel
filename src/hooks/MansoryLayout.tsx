...
// code was спизджений from https://github.com/askides/react-plock
...
export type MasonryProps<T> = React.ComponentPropsWithoutRef<"div"> & {
  items: T[];
  render: (item: T, rowIdx: number, columnIdx: number, columns: number) => React.ReactNode;
  config: {
    ...
    >
      {dataColumns.map((column, columnIdx,) => (
        <MasonryRow gap={gap} key={columnIdx}>
          {column.map((item, rowIdx) => render(item, rowIdx, columnIdx, columns))}
        </MasonryRow>
      ))}
    </div>
  );
